...
module.exports = function (emitter, options, collectionRunOptions) {
  ...
  emitter.on('beforeItem', (err, o) => {
    ...
    console.log(o.item.request.description ? o.item.request.description : 'SEM_TAG_NA_REQUEST');
    const testObj = client.startTestItem({
      name: o.item.name,
      type: 'TEST',
      tags: o.item.request.description ? o.item.request.description.content.split('\r\n').filter(i => !!i).map(i => i.toString().trim()).filter(i => !!i) : []
    },
      launchObj.tempId);
  ...
  emitter.on('beforeRequest', (err, o) => {
    ...
    const testObj = collectionMap.get(o.cursor.ref),
      // Name of the current step. If request doesn't have description - default name will be used
      //stepName = o.request.description ? o.request.description.content : DEFAULT_REQUEST_STEP_NAME,
      stepName = DEFAULT_REQUEST_STEP_NAME,
      // Creates a new step
    ...
    /*	var requestJson = o.request;
      if (isJson(requestJson.body.raw)){
        requestJson.body = JSON.parse(requestJson.body.raw);
      }*/
    logMessageWithFile(stepObj.tempId, o.request, o.item.name + "-REQUEST.json");
  ...
  function logMessageWithFile(id, request, nomeArquivo, level = 'INFO') {
    client
      .sendLog(id, {
        level: level,
        message: nomeArquivo
        ...
        content: Buffer.from(JSON.stringify(request)).toString('base64')
      })
      .promise.catch(errorHandler);
  }
  ...
  function finishSteps(reference, testObj, run) {
    ...
    var jsonResponse = execution.response;
    var dataResponse = decoder.write(jsonResponse.stream);
    //if isJson(dataResponse){
    //	jsonResponse.data = JSON.parse(dataResponse);
    //}else{
    jsonResponse.data = dataResponse;
    //};
    //jsonResponse.data = decoder.write(jsonResponse.stream);
    jsonResponse.stream = "";

    logMessageWithFile(requestStep.stepId, jsonResponse, execution.item.name + "-RESPONSE.json");
  ...
  function getTags(variables) {
    ...
    return tags && tags.value ? tags.value.split(';').filter(i => !!i).map(i => i.toString().trim()).filter(i => !!i) : [];
  }
};
